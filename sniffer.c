...
void rec_loop(void (*handle_frame)(unsigned char *, unsigned int)) {
  ...
  while (1) {
    ssize_t rbytes =
        recvfrom(rs, buf, BUF_MAX, 0, (struct sockaddr *)&addr, &addrlen);
    if (rbytes == -1) {
...
void print_up(unsigned char *buffer, unsigned int size) {
  ...
  if (iph->protocol == IPPROTO_TCP) {
    ...
    printf(" |*		TCP header\n");
    printf(" |* Source %s : %d\t>-->\t", inet_ntoa(saddr.sin_addr),
           ntohs(tcph->source));
    ...
    if (size - (ETH_HLEN + (iph->ihl) * 4 + (tcph->doff) * 4)) {
      data_print(buffer + ETH_HLEN + (iph->ihl) * 4 + (tcph->doff) * 4,
                 size - (ETH_HLEN + (iph->ihl) * 4 + (tcph->doff) * 4));
    }
  }
  if (iph->protocol == IPPROTO_UDP) {
    ...
    if (size - (ETH_HLEN + (iph->ihl) * 4 + sizeof(udph))) {
      data_print(buffer + ETH_HLEN + (iph->ihl) * 4 + sizeof(udph),
                 size - (ETH_HLEN + (iph->ihl) * 4 + sizeof(udph)));
    }
  }
  total++;
  printf(
...
void log_up(unsigned char *buffer, unsigned int size) {
  ...
  fprintf(fp, "%d/%d/%d:%d:%d  ", 1 + tptr->tm_mon, tptr->tm_mday,
          tptr->tm_hour, tptr->tm_min, tptr->tm_sec);
  fprintf(fp, "%.2X:%.2X:%.2X:%.2X:%.2X:%.2X >-> ", eth->h_source[0],
          eth->h_source[1], eth->h_source[2], eth->h_source[3],
          eth->h_source[4], eth->h_source[5]);
  fprintf(fp, "%.2X:%.2X:%.2X:%.2X:%.2X:%.2X ", eth->h_dest[0], eth->h_dest[1],
