...
void logger(unsigned char*, unsigned int);
void parse(int, char**);
void sigkill(int);
void sigchld(int);
...
void print_up(unsigned char* buffer, unsigned int size) {
	...
	if (iph->protocol == IPPROTO_TCP) {
		...
		if (size - (ETH_HLEN + (iph->ihl)*4 + (tcph->doff)*4)) {
			print_data(buffer+ETH_HLEN+(iph->ihl)*4+(tcph->doff)*4, size-(ETH_HLEN+(iph->ihl)*4+(tcph->doff)*4));
		}				
	}
	...
	if (iph->protocol == IPPROTO_UDP) {
		...
		if (size - (ETH_HLEN + (iph->ihl)*4 + sizeof(udph))) {
			print_data(buffer+ETH_HLEN+(iph->ihl)*4+sizeof(udph), size-(ETH_HLEN+(iph->ihl)*4+sizeof(udph)));
		}
	}
	total++;
...
/*Logging mode*/
void logger(unsigned char* buffer, unsigned int size) {
...
int main(int argc, char *argv[]) {
	...
	int var = setsockopt(rs, SOL_SOCKET, SO_BINDTODEVICE, ifname, strlen(ifname));
	
	if (var < 0) {
		printf("Device %s binding error\n", ifname);
	}	
	strncpy(ifr.ifr_name, ifname, IF_NAMESIZE);
	
	/*Switch to the promiscuous mode*/
	if (ioctl(rs, SIOCGIFFLAGS, &ifr)) {
		printf("Interface %s status reading error\n", ifname);
	} else {
			ifr.ifr_flags |= IFF_PROMISC;
				if (ioctl(rs, SIOCGIFFLAGS, &ifr) < 0) {
					printf("Interface %s status setting error\n", ifname);
				}
